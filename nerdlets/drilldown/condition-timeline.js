...
const ConditionTimeline = ({ timeline, timeRange }) => {
  ...
  const generateTimeChunks = (format) => {
    ...
    if (timeRange?.duration <= 86400000) {
      ...
      timeRange?.duration > 86400000 &&
      timeRange?.duration <= 1209600000
    ) {
  ...
  const calculateXPosition = (ts) => {
    ...
    for (let x = 0; x < timeChunkLengths.length - 1; x++) {
      ...
      if (ts >= chunkStart.timestamp && ts < chunkEnd.timestamp) {
        ...
        xPos =
          chunkStart.position +
          posRatio * (chunkEnd.position - chunkStart.position);
        break;
      }
    }
  ...
  const renderTimeChunks = () => {
    ...
    if (timeChunks?.length > 0) {
      return timeChunks.map((tc, i) => {
        let splitTc = tc.split(',');
        if (i == 0) {
          return (
            <div key={i}>
              <div className="timeChunks" style={{ left: '1%' }}>
                {splitTc[0]}
              </div>
              <div
                className="timeChunks"
                style={{ left: '1%', marginTop: '15px' }}
              >
                {splitTc[1]}
              </div>
            </div>
        ...
        if (i + 1 === timeChunks.length) {
          return (
            <div key={i}>
              <div className="timeChunks" style={{ left: '97.5%' }}>
                {splitTc[0]}
              </div>
              <div
                className="timeChunks"
                style={{ left: '97%', marginTop: '15px' }}
              >
                {splitTc[1]}
              </div>
            </div>
        ...
        return (
          <div key={i}>
            <div className="timeChunks" style={{ left: `${tcPos}%` }}>
              {splitTc[0]}
            </div>
            <div
              className="timeChunks"
              style={{ left: `${tcPos}%`, marginTop: '15px' }}
            >
              {splitTc[1]}
            </div>
          </div>
    ...
    return (
      <>
        <div className="barContainer">
          {Object.keys(timeline).map((k) => {
            if (timeline[k].length > 0) {
              return timeline[k].map((time) => {
                return renderMarker(k, time);
              });
            }
          })}
        </div>
        <div style={{ display: 'inline' }}>{renderTimeChunks()}</div>
