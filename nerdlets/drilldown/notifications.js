...
import { chunkData, pluckTagValue, getCardColor, getTooltip, generateDayChunks, getIssues, getNotifications, getWorkflows, getDestinations, getDestinationRelationships } from '../shared/utils';
...
const Notifications = ({ selectedAccount }) => {
  ...
  if (!loading && issues !== null && workflows !== null && destinations !== null) {
    return (
      <div id="drilldown">
        <Card.Group style={{textAlign: 'center'}} id="card-group" itemsPerRow={3}>
          <Card color={getCardColor(Number(issues.unsentPercent))} onClick={() => setSelectedCard('unsent_issues')}>
            <Card.Header>
              <h3>
              <Tooltip
          ...
          <Card color={getCardColor(Number(destinations.unusedPercent))} onClick={() => setSelectedCard('unused_dests')}>
            <Card.Header>
              <h3>
              <Tooltip
            ...
            <Card.Content>
              <Statistic color={getCardColor(Number(destinations.unusedPercent))}>
                <Statistic.Value>{Number(destinations.unusedPercent)}%</Statistic.Value>
              </Statistic>
            </Card.Content>
          </Card>
          <Card color={getCardColor(Number(workflows.dupePercent))} onClick={() => setSelectedCard('overlap_workflows')}>
            <Card.Header>
              <h3>
              <Tooltip
        ...
        {selectedCard === 'unsent_issues' ? <Issues selectedAccount={selectedAccount} issues={issues}/> : ''}
        {selectedCard === 'unused_dests' ?  <Destinations selectedAccount={selectedAccount} destinations={destinations}/> : ''}
        {selectedCard === 'overlap_workflows' ? <Workflows workflows={workflows}/> : ''}
      </div>
    );
  }
