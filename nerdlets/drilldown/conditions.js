...
import { Button, Dropdown, DropdownItem, LineChart, navigation, PieChart, TableChart, Table, TableHeader, TableHeaderCell, TableRow, TableRowCell, TextField, Tooltip, Spinner } from 'nr1';
import { Icon } from 'semantic-ui-react';
import { getConditions, getConditionTimeline, getTooltip, pluckTagValue } from '../shared/utils';
...
const ConditionHistory = ({ selectedAccount, timeRange, policies }) => {
  ...
  const renderPolicyDropdown = useMemo(() => {
    if (policies && policies.length > 0) {
      ...
      return (
        ...
        >
        {({ item }) => <DropdownItem key={item.id} onClick={() => fetchAndSetConditions(item)}>{item.name}</DropdownItem>}
        </Dropdown>
      );
    }
    return '';
  ...
  const renderConditionDetail = useMemo(() => {
          ...
          <div style={{marginTop: '15px'}}>
            <div id='entity-violation' className="conditionCharts" style={{marginRight: '50px'}}>
              ...
              <PieChart
                style={{display: 'inline-block'}}
                accountIds={[selectedAccount.accountId]}
                query={`FROM NrAiIncident SELECT count(*) where event = 'open' and conditionId = ${Number(selectedCondition.id)} facet if(entity.guid = '' or entity.guid is null, targetName, entity.name) as 'entity' ${timeClause} LIMIT 50`}
                fullWidth
              />
            </div>
    ...
    return '';
  }, [selectedAccount, timeRange, policies, selectedCondition, conditionTimeline, selectedPolicy])
